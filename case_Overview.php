...
$number =$_GET['number'];//此變數用來得知年份ID
...
$s_action=$_POST["s_action"];//判斷要總覽還式搜尋的變數,這是要搜尋
...
if($action=="view"){//總覽
	//印出該年份底下所有內容
	$sql="SELECT * FROM bag where relatedID = '{$number}'";
	...
	if (mysql_num_rows($result) >0){
		...
		while ($row = mysql_fetch_array($result)){
			echo "<tr><td class='i2' align='center' valign='middle'>{$c_number}</td>";
			for($i=3;$i<=20;$i++){
	...
	if(empty($implode)){
		...
		$sql= " SELECT *FROM `bag` WHERE `relatedID` = '{$number}' and " . implode(" and ", $implode);
	}
	else{//這是一般搜尋
		
		$sql= " SELECT *FROM `bag` WHERE `relatedID` = '{$number}' and " . implode(" or ", $implode);
	}
		$result = mysql_query($sql);
		echo "<!--startprint1-->";
		...
		while($row = mysql_fetch_array($result)){
			echo "<tr>";
			echo "<td align='center' valign='middle'>{$c_number}</td>";
			if($sdhfk==false){
				$sdhfk=true;
			}
			for($i=3;$i<=20;$i++){
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
	  <!--form在這,看到了嗎?-->
        <form role="form" id="srch_form" action="case_Overview.php?number=<?php echo $number; ?>" method="post">

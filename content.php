...
$optionsRadios=$_POST["optionsRadios"];//判斷用and搜還or
// }
//_GET

// $srch_condate=$_GET["srch_condate"];
// $srch_projector=$_GET["srch_projector"];
// $srch_category=$_GET["srch_category"];
// $srch_outpeople=$_GET["srch_outpeople"];
// $optionsRadios=$_GET["optionsRadios"];//判斷用and搜還or

//$s_action=$_GET["s_action"];//判斷要總覽還式搜尋的變數,這是要搜尋
?>
<!DOCTYPE html>
...
<head>
	...
	<script src="JS/content_JS.js" type="text/javascript"></script><!--此檔的JS碼-->
	<link rel="stylesheet" type="text/css" href="CSS/content_CSS.css" /><!--此檔的CSS碼-->
</head>
<body>
	<div id="container"><!--頂端功能列開始-->
		<nav class="nav navbar-default">
			<div class="container-fluid">
				<ul class="nav navbar-nav">
					<li><a href="year.php"><span class="glyphicon glyphicon-home"></span>首頁</a></li>
					<li><a href="case.php?id=<?php echo $positionid ?>"><span class="glyphicon glyphicon-arrow-left"></span>上一頁</a></li>
					<li><a href="indeXa.php?anme=add&&newcaseid=<?php echo $conid; ?>&&cyear=<?php echo $cyear; ?>&&positionid=<?php echo $positionid; ?>&&caseid=<?php echo $caseid; ?>&&action=<?php echo $action; ?>"><span class="glyphicon glyphicon-plus"></span>建立新內容</a></li>
					<li><a href="content_PrintMode.php?conid=<?php echo $conid ?>" ><span class="glyphicon glyphicon-print"></span><span style="cursor:pointer">列印模式</span></a></li>
...
if ($conid !='' ){
		...
		$row_3=mysql_fetch_array($result);
			...
			echo "<h4><a href='year.php'>$cyear</a>➤<a href='case.php?id=$positionid'>$concaid</a><br></h4>";
}
if ($action=="view"){
		...
		while ($row = mysql_fetch_array($result)){//將內容用迴圈列印出來
			...
			if($_SESSION["memberLevel"] >= 2){
				echo "<td><input type=submit name=ok value='刪除' onclick=\"if(confirm('您確定送出嗎?')) {
							window.location.href='content_delete.php?del={$row['t_number']}&&newcaseid={$conid}&&cyear=$cyear&&positionid=$positionid&&caseid=$caseid&&action=$action';return true}else return false\">
					  </td>
					  <td><input type='button' value='編輯' onclick=\"self.location.href='indeXa.php?edit={$row['t_number']}&&anme=edit&&newcaseid={$conid}&&cyear=$cyear&&positionid=$positionid&&caseid=$caseid&&action=$action'\">
					  </td>";
					  echo "</tr>";
					  echo "</p>";
			}
	...
	if($have_content == null){
		echo "<p>目前{$concaid}裡並沒有內容哦！請新增新內容哦！<br>";
	}
	else{//計算目前資料總筆數,如果有內容才計算
		$sql="select * from `bag` WHERE `id` = '{$conid}'";
		$result=mysql_query($sql);
		$row = mysql_num_rows($result);
		echo "<div2>目前總共有{$row}筆</div2>";
	}
}
else if($action=="serch"){
	echo $action;
	echo $srch_category;
	$sdhfk =false;
	$implode = array();
	...
	if(empty($implode)){
		...
		$sql= " SELECT *FROM `bag` WHERE `id` = '{$conid}' and " . implode(" and ", $implode);
	}
	else{//這是一般搜尋
		
		$sql= " SELECT *FROM `bag` WHERE `id` = '{$conid}' and " . implode(" or ", $implode);
	}
		$result = mysql_query($sql);
		echo "<table border='1' class='table table-striped table-bordered table-hover table-condensed tablesorter' id='myTable'>";
		echo "<thead>";
		...
		while($row = mysql_fetch_array($result)){
			echo "<tr>";
			echo "<td class='i2' align='center' valign='middle'>{$c_number}</td>";
			if($sdhfk==false){
				$sdhfk=true;
			}
			for($i=3;$i<=20;$i++){
	...
	<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	  <div class="modal-dialog">
		<div class="modal-content">
		  ...
		  <div class="modal-body">
		  <!--form在這,看到了嗎?-->
			<form role="form" id="srch_form" action="content.php?conid=<?php echo $conid; ?>&&cyear=<?php echo $cyear; ?>&&positionid=<?php echo $positionid; ?>&&caseid=<?php echo $caseid; ?>&&action=serch" method="post">
